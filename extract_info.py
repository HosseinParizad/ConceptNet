"""
MIMIC-IV Feature Extraction Pipeline — v1.3.1  (LEAKAGE-FIXED + BUGS-FIXED)
=============================================================================
Changes vs v1.3
  FIX-1  load_base_tables()  adm_age clip
             adm_age = (admittime.year - yob).clip(upper=91)
             Old code could yield age > 91 for the ≥91-capped cohort because
             admittime.year can exceed anchor_year by several years.

  FIX-2  build_c5()  first_val never pivoted → AKI columns were 100% NaN
             Added 'first_val' → suffix 'first' to the pivot metric loop.
             Corrected AKI baseline reference:
               lab_creatinine_first_val_bw  →  lab_creatinine_first_bw

  FIX-3  main()  Physiological clip rules applied before CSV save
             Values outside plausible ranges set to NaN (not boundary-clipped)
             for vitals (pdw window), labs (bw window), and OMR measures.

Leakage fixes carried forward from v1.3 (unchanged)
  LEAK-01  adm_discharge_location removed
  LEAK-02  adm_los_days not forwarded as model feature
  LEAK-03  adm_inhospital_mortality is label only
  LEAK-04  util_prior_hadm_count_* removed
  LEAK-05  trans_last_careunit removed; icu_total_los_days → _dpt suffix
  LEAK-06  Charlson from prior admissions only
  LEAK-07  Whole-stay (_ws) lab columns removed; bw + pdw windows only
  LEAK-08  note_stable_at_discharge, note_resolution_score removed
"""
...
CHARLSON_WEIGHTS = {
    ...
    "rheum": 1, "pud": 1, "mild_liver": 1, "diabetes": 1,
    "hemi_para": 2, "renal": 2, "cancer": 2, "diabetes_comp": 2,
    "mod_severe_liver": 3, "metastatic": 6, "hiv": 6,
}

# ─────────────────────────────────────────────────────────────────────────────
# 2c) MEDICATION NORMALISATION & HIGH-RISK CLASSES
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 2d) PHYSIOLOGICAL CLIP RULES  (FIX-3)
# Values outside these ranges are MIMIC charting errors; set to NaN before saving.
# We null rather than clip-to-boundary to avoid feeding boundary artifacts into models.
# ─────────────────────────────────────────────────────────────────────────────
CLIP_RULES = {
    # Labs — bw window
    'lab_wbc_last_bw':         (0,   500),
    'lab_wbc_min_bw':          (0,   500),
    'lab_wbc_max_bw':          (0,   500),
    'lab_wbc_mean_bw':         (0,   500),
    'lab_sodium_last_bw':      (100, 190),
    'lab_sodium_min_bw':       (100, 190),
    'lab_sodium_max_bw':       (100, 190),
    'lab_sodium_mean_bw':      (100, 190),
    # Vitals — pdw window
    'vital_hr_last_pdw':       (10,  350),
    'vital_hr_min_pdw':        (10,  350),
    'vital_hr_max_pdw':        (10,  350),
    'vital_hr_mean_pdw':       (10,  350),
    'vital_sbp_last_pdw':      (40,  300),
    'vital_sbp_min_pdw':       (40,  300),
    'vital_sbp_max_pdw':       (40,  300),
    'vital_sbp_mean_pdw':      (40,  300),
    'vital_dbp_last_pdw':      (10,  200),
    'vital_dbp_min_pdw':       (10,  200),
    'vital_dbp_max_pdw':       (10,  200),
    'vital_dbp_mean_pdw':      (10,  200),
    'vital_map_last_pdw':      (20,  200),
    'vital_map_min_pdw':       (20,  200),
    'vital_map_max_pdw':       (20,  200),
    'vital_map_mean_pdw':      (20,  200),
    'vital_spo2_last_pdw':     (50,  100),
    'vital_spo2_min_pdw':      (50,  100),
    'vital_spo2_max_pdw':      (50,  100),
    'vital_spo2_mean_pdw':     (50,  100),
    'vital_rr_last_pdw':       (1,    80),
    'vital_rr_min_pdw':        (1,    80),
    'vital_rr_max_pdw':        (1,    80),
    'vital_rr_mean_pdw':       (1,    80),
    'vital_temp_last_pdw':     (25,   45),
    'vital_temp_min_pdw':      (25,   45),
    'vital_temp_max_pdw':      (25,   45),
    'vital_temp_mean_pdw':     (25,   45),
    # OMR
    'omr_bmi_last_pre':        (10,  100),
    'omr_weight_kg_last_pre':  (2,   500),
    'omr_height_cm_last_pre':  (50,  250),
}
...
def _apply_clip_rules(df: pd.DataFrame) -> pd.DataFrame:
    """FIX-3: Null physiologically impossible values before saving."""
    nulled = 0
    for col, (lo, hi) in CLIP_RULES.items():
        ...
        if n:
            ...
            nulled += n
    log.info(f"  Clip rules: {nulled:,} out-of-range values set to NaN")
    return df
...
def load_base_tables() -> pd.DataFrame:
    ...
    # FIX-1: clip(upper=91) prevents year-difference from exceeding 91 for
    # the ≥91-capped cohort when admittime.year > anchor_year.
    base["adm_age"] = (base["admittime"].dt.year - base["yob"]).clip(upper=91)
    base.loc[base["anchor_age"] >= 91, "adm_age"] = 91

    # Compute readmission label BEFORE dropping any future admissions
    ...
    base['readmitted_30d'] = (
        (base['days_to_readmit'] > 0) & (base['days_to_readmit'] <= 30)
    ).astype(int)
    ...
    # adm_los_days used only for cohort filtering — never forwarded as a feature (LEAK-02)
    base['adm_los_days'] = (
        (base['dischtime'] - base['admittime']).dt.total_seconds() / 86400.0
    )
    base = base[base['adm_los_days'] >= 0.5].copy()
    log.info(f"  After min LOS >= 0.5 d:    {len(base):,}")
...
def build_c1(base: pd.DataFrame) -> pd.DataFrame:
    """
    LEAK-01: adm_discharge_location removed (encodes "DIED", "HOSPICE" etc.)
    LEAK-02: adm_los_days not forwarded (whole-stay value unknown at admission)
    LEAK-03: adm_inhospital_mortality is label only — exclude from X when training
    """
    ...
    # ED LOS capped at admittime so it never includes time post-admission
    ed_hours = np.where(
    ...
    df['adm_ed_los_hours'] = np.where(
        np.array(ed_hours, dtype=float) < 0, np.nan, ed_hours)
...
def build_c2(base: pd.DataFrame) -> pd.DataFrame:
    """
    LEAK-04: util_prior_hadm_count_* removed — always 0 after first-admission filter.
    KEPT:    util_days_since_last_discharge (structurally NaN for first admissions).
    KEPT:    util_prior_ed_count_30d / _180d (from edstays, valid).
    """
    ...
    if os.path.exists(FILE_PATHS['edstays']):
        ...
        ed = pd.read_csv(FILE_PATHS['edstays'],
                         usecols=['subject_id','intime'],
                         parse_dates=['intime'])
...
def build_c3(base: pd.DataFrame) -> pd.DataFrame:
    """
    LEAK-05: trans_last_careunit removed (leaks discharge/death careunit).
             trans_first_careunit kept.
    ...
    tr_ws = tr[
        (tr['intime'] >= tr['admittime']) & (tr['intime'] <= tr['dischtime'])
    ].copy()

    feats = (
        tr_ws.sort_values(['hadm_id','intime'])
             .groupby('hadm_id')
             .agg(
                 icu_any_transfer=(
                     'careunit',
    ...
    chg = (
        tmp.assign(changed=lambda x: (x['careunit'] != x['prev']).astype(int))
    ...
    if os.path.exists(FILE_PATHS['icustays']):
        ...
        icu_agg = (
            icu.groupby('hadm_id')
               .agg(
                   ...
                   icu_total_los_days_dpt=('los', 'sum'),
        ...
        icu_agg['icu_time_to_first_icu_hours'] = (
            (icu_agg['icu_first_intime'] - icu_agg['admittime']
             ).dt.total_seconds() / 3600.0
        ).clip(lower=0)
...
def build_c4(base: pd.DataFrame) -> pd.DataFrame:
    """
    LEAK-06: Charlson computed from PRIOR admissions only.
             dx/proc COUNT features from this admission (_ws) are kept and labelled clearly.
    """
    ...
    # Attach subject_id + dischtime from the admission that generated each code
    dx_all = dx_all.merge(
        all_adm[['hadm_id','subject_id','dischtime']], on='hadm_id', how='inner')

    # Join with index admission times and keep only codes from PRIOR admissions
    ...
    # Count features from THIS admission's codes (valid at discharge-coding time)
    ...
    pr = pd.read_csv(FILE_PATHS['procedures'],
...
def build_c5(base: pd.DataFrame) -> pd.DataFrame:
    """
    LEAK-07: Whole-stay (_ws) lab columns removed; bw (first 24h) and pdw (last 24h) only.

    FIX-2:  'first_val' added to the pivot metric loop so lab_creatinine_first_bw
            is created, enabling the AKI baseline column to be populated.
            AKI baseline reference corrected:
              lab_creatinine_first_val_bw  →  lab_creatinine_first_bw
    """
    ...
    for chunk in tqdm(reader, desc="  labevents chunks", unit="chunk"):
        ...
        bw_mask  = chunk['charttime'] <= chunk['bw_end']
        pdw_mask = chunk['charttime'] >= chunk['pdw_start']

        bw_df        = chunk.loc[bw_mask,  ['hadm_id','analyte','charttime','valuenum','flag']].copy()
        bw_df['win'] = 'bw'
        pdw_df        = chunk.loc[pdw_mask, ['hadm_id','analyte','charttime','valuenum','flag']].copy()
        pdw_df['win'] = 'pdw'
        ...
        agg = (
            dfw.groupby(['hadm_id','analyte','win'])
        ...
        if acc.empty:
            ...
            new = agg.set_index(key_cols)[
                ['v_min','v_max','v_sum','v_cnt','abn_cnt',
                 'last_time','last_val','first_time','first_val']
            ]
            ...
            # last_time / last_val — keep the later timestamp
            a_t = acc['last_time']
            n_t = new['last_time']
            take_new = (
                (a_t.isna() & n_t.notna()) |
            ...
            # first_time / first_val — keep the earlier timestamp
            a_f = acc['first_time']
            n_f = new['first_time']
            take_new_f = (
                (a_f.isna() & n_f.notna()) |
    ...
    # Slope + delta for PDW (needs ≥2 measurements)
    ...
    def colname(suffix, analyte, win):
        return f'lab_{analyte}_{suffix}_{win}'

    # FIX-2: Added 'first_val' → 'first' to the pivot loop.
    # Without this, lab_creatinine_first_bw never existed and the AKI baseline
    # column was 100% NaN.
    PIVOT_METRICS = {
        'last_val':  'last',
        'first_val': 'first',
    ...
    for win in ['bw', 'pdw']:
        ...
        if sub.empty:
            continue
        for col_key, suffix in PIVOT_METRICS.items():
            p = sub.pivot(index='hadm_id', columns='analyte', values=col_key)
            p.columns = [colname(suffix, a, win) for a in p.columns]
            wide_parts.append(p)
    ...
    # FIX-2: corrected column name 'lab_creatinine_first_bw' (not 'first_val_bw')
    lab_wide['aki_creat_baseline_bw'] = lab_wide.get(
        'lab_creatinine_first_bw', pd.Series(np.nan, index=lab_wide.index))
    lab_wide['aki_creat_max_bw'] = lab_wide.get(
        'lab_creatinine_max_bw', pd.Series(np.nan, index=lab_wide.index))
...
def build_c6(base: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Whole-stay prescription counts ──────────────────────────────────────
    presc_agg_rows = []
    ...
    for chunk in tqdm(presc_reader, desc="  prescriptions chunks", unit="chunk"):
        ...
        chunk['hadm_id'] = chunk['hadm_id'].astype(int)
        chunk = chunk.merge(
            times[['hadm_id','admittime','dischtime','medwin_start']],
            on='hadm_id', how='inner')
        ws = chunk[
            (chunk['starttime'] >= chunk['admittime']) &
    ...
    presc_agg = (
        pd.concat(presc_agg_rows, ignore_index=True)
          .groupby('hadm_id', as_index=False).sum()
        if presc_agg_rows
        else pd.DataFrame(columns=['hadm_id','med_total_orders_count_ws','med_prn_count_ws'])
    )

    # ── Pre-discharge 24h medication window ─────────────────────────────────
    medwin_parts = []
    for src_file, usecols_src, drug_col, has_freq in [
        (FILE_PATHS['prescriptions'],
         ['hadm_id','drug','route','starttime','stoptime'],
         'drug', False),
        (FILE_PATHS['pharmacy'],
        ...
        reader2 = pd.read_csv(
            src_file, usecols=usecols_src,
            parse_dates=['starttime','stoptime'],
            chunksize=CHUNK_SIZE, low_memory=False)

        for chunk in tqdm(reader2,
                          desc=f"  {os.path.basename(src_file)} medwin", unit="chunk"):
            chunk = chunk[chunk['hadm_id'].notna()].copy()
            if chunk.empty:
                continue
            chunk['hadm_id']   = chunk['hadm_id'].astype(int)
            ...
            chunk = chunk.merge(
                times[['hadm_id','dischtime','medwin_start']],
                on='hadm_id', how='inner')
            med = chunk[
                (chunk['starttime'] <= chunk['dischtime']) &
    ...
    if medwin_parts:
        ...
        agg_d  = {
            'med_unique_count_medwin':                 'max',
            'med_num_routes_medwin':                   'max',
        ...
        hr_cols = [f'med_highrisk_{cls}_medwin' for cls in HIGH_RISK_CLASSES]
        ...
        for c in (
            ['med_unique_count_medwin','med_num_routes_medwin',
             'med_frequency_complexity_score_medwin','med_highrisk_class_count_medwin'] +
            [f'med_highrisk_{cls}_medwin' for cls in HIGH_RISK_CLASSES]
        ):
            medwin[c] = np.nan
...
# ─────────────────────────────────────────────────────────────────────────────
# 11) C7 — OMR (pre-admission body measures)
# ─────────────────────────────────────────────────────────────────────────────
def build_c7(base: pd.DataFrame) -> pd.DataFrame:
    ...
    # Unit normalisation: weight > 200 → assume lbs, convert to kg
    omr_w['result_value_num'] = np.where(
        omr_w['result_value_num'] > 200,
        omr_w['result_value_num'] * 0.453592,
        omr_w['result_value_num'])
    # Height < 100 → assume inches, convert to cm
    omr_h['result_value_num'] = np.where(
        omr_h['result_value_num'] < 100,
        omr_h['result_value_num'] * 2.54,
        omr_h['result_value_num'])

    def pick_closest_pre(sub_df, t_admit):
        t_start = t_admit - pd.Timedelta(days=365)
        sub = sub_df[
            (sub_df['chartdate'] < t_admit) & (sub_df['chartdate'] > t_start)
        ].dropna(subset=['result_value_num'])
        if sub.empty:
            return np.nan
        return float(sub.loc[(t_admit - sub['chartdate']).abs().idxmin(),
                              'result_value_num'])
    ...
    for _, row in tqdm(base[['hadm_id','subject_id','admittime']].iterrows(),
        ...
        # Compute BMI from weight + height if direct BMI is missing or implausible
...
# ─────────────────────────────────────────────────────────────────────────────
# 12) C8 — ICU CHART VITALS + GCS (pdw window only)
# ─────────────────────────────────────────────────────────────────────────────
def build_c8(base: pd.DataFrame) -> pd.DataFrame:
    log.info("C8 — ICU chart vitals + GCS (chunked, pdw window) …")
    ...
    for chunk in tqdm(reader, desc="  chartevents chunks", unit="chunk"):
        ...
        chunk = chunk.merge(times, on='hadm_id', how='inner')
        chunk = chunk[
            (chunk['charttime'] >= chunk['pdw_start']) &
            (chunk['charttime'] <= chunk['dischtime'])
        ]
        if chunk.empty:
            continue
        ...
        # Convert Fahrenheit → Celsius for itemid 223761
        ...
        agg = (
            vit_chunk.groupby(['hadm_id','vital'])
                     .agg(v_min=('valuenum','min'), v_max=('valuenum','max'),
...
# LEAK-08: note_stable_at_discharge and note_resolution_score removed
# (clinical outcome proxies — fire on "stable", "resolved", "worsening" etc.)
...
def build_d(base: pd.DataFrame) -> pd.DataFrame:
    ...
    reader     = pd.read_csv(FILE_PATHS['discharge'],
...
def main():
    log.info("=" * 64)
    log.info(" MIMIC-IV Feature Extraction Pipeline v1.3.1 (leakage+bugs fixed)")
    log.info("=" * 64)
    ...
    log.info(f"Final shape: {final.shape[0]:,} rows × {final.shape[1]:,} cols")

    # FIX-3: Null physiologically impossible values (charting errors in MIMIC)
    log.info("Applying physiological clip rules …")
    final = _apply_clip_rules(final)

    # Missing value audit
    miss = final.isna().mean().sort_values(ascending=False)
    high = miss[miss > 0.5]
    if not high.empty:
        log.info(f"Columns with >50% missing ({len(high)} total):")
